...
def generate_copil_text(context: dict, return_raw: bool = False):
    ...
    # --- PROMPT ---
    prompt = f"""
    Je suis un chef de projet IT et je dois présenter le slide ppt aux autres chefs de projet.
    Fais l'analyse et donne les justifications des écarts de budgets.

    Si tu n’es pas capable de produire un JSON strict, renvoie EXACTEMENT :
    {{"error": "format"}}
    ...
    # ------------------------------------------------------------------
    # 🔥 Extraction robuste du JSON (ajout minimal, code d’origine préservé)
    # ------------------------------------------------------------------

    # 1) Supprimer uniquement les backticks, pas le contenu JSON
    cleaned = raw_text.replace("```json", "").replace("```", "").strip()


    # 2) Trouver le premier JSON complet
    json_pattern = r"\{(?:[^{}]|(?:\{[^{}]*\}))*\}"
    matches = re.findall(json_pattern, cleaned, flags=re.DOTALL)
